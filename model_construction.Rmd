...
anova(fit_tbbc18_2, fit_tbbc18_5) # 2 is better
...
anova(fit_tbbc19_cf_4,fit_tbbc19_cf_3) # no dfiffference, we can use the block design
```
...
library(readxl)
z025_19_raw <- read.csv("z025_19_raw.csv") # with raw data information for a population with 2 differnt fields
tbbc3_19_raw <- read.csv("tbbc3_19_raw.csv") 
...
library(ggplot2)
c19$range <- as.numeric(as.character(c19$range))
c19$row <- as.numeric(as.character(c19$row))
p_c19 <- ggplot(c19, aes(x=range, y=row)) + 
  geom_tile(aes(fill=mean)) +
  theme_bw() + theme(panel.grid = element_blank()) +
  scale_fill_gradient(low = "#F4D27A", high = "#525f74") + 
  scale_x_continuous(breaks = seq(2,9,1)) +
  scale_y_continuous(breaks = seq(5,76,5)) +
  ggtitle("Plot mean of 2 populations in 2019 CFAR-200") +
  labs(fill = "Plot mean") + 
  xlab("range number") + 
  ylab("row number") 
 p_c19 
...
# plot the cfar 2019 field, both z025 and tbbc3 populaiton
library(ggplot2)
m19$range <- as.numeric(as.character(m19$range))
m19$row <- as.numeric(as.character(m19$row))
p_m19 <- ggplot(m19, aes(x=range, y=row)) + 
  ...
  ggtitle("Plot mean of 2 populations in 2019 MF-700") +
  labs(fill = "Plot mean") + 
  xlab("range number") + 
  ylab("row number") 
 p_m19 
